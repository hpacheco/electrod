...
module Make_SMV_file_format (Ltl : Solver.LTL) :
  ...
  let pp_plain_decl vartype out name tuple_str =
    Fmtc.hardline out ();
    let name_str = Name.to_string name in
    Fmtc.pf out "%s %s-%s : boolean;@\n" vartype name_str tuple_str

  let pp_partial_tuple vartype out name (pt : Enums.partial_tuple) =
      let name_str = Name.to_string name in
      let tuple_lst = List.concat (List.map Enums.partial_atom_to_string pt) in
      match Enums.partial_vals pt with
      | None ->
                  ...
                  if (Stdlib.(==) sz 1)
                      then
                          (Tuple_set.iter (fun t ->
                              let tuple_str = Enums.tuple_to_string (Enums.tuple_to_full t pt) in
                              ...
                              (
                              let s = Fmt.str "__%s-%a" name_str (Fmt.list ~sep:(Fmt.any "-") Fmt.string) tuple_lst in
                              Ltl.Atomic.register_enum s name pt;
                              Fmtc.(pf out "%s %s : 0..%n;@\n" vartype s (sz-1));
                              Tuple_set.iter (fun t ->
                                  let tuple_str = Enums.tuple_to_string (Enums.tuple_to_full t pt) in
                                  let tuple_idxs = Valuations_list.indices_of t vs in
  ...
  let pp_sup vartype out name (sup : Scope.sup_t) (tuples : Tuple_set.t) =
      Iter.iter (pp_partial_tuple vartype out name) (Enums.mk_partial_tuples sup tuples)

  let pp_decl elo vartype out (name : Name.t) (tuples : Tuple_set.t option) =
    ...
    match scope with
        ...
        | Inexact (inf,sup) ->
            ...
            pp_sup vartype out name sup (Tuple_set.diff sup_tuples inf)
  ...
  let pp_used_decls elo vartype out (atoms : Ltl.Atomic.t Iter.iter) =
      ...
      Name.Map.iter (fun key value ->
          Fmtc.hardline out ();
          pp_decl elo vartype out key (Some value))
          gatoms

  let pp_all_decls elo out =
      let decls = Domain.to_list elo.Elo.domain in
      List.iter (fun (name,rel) ->
          let vartype = (match rel with
              | Relation.Const _ -> "FROZENVAR"
              | Relation.Var _ -> "VAR")
          in pp_decl elo vartype out name None
      ) decls
  ...
  let pp_count_variables ~only_used ?(margin = 80) out
      { elo; init; invariant; trans; property } =
    let open Fmtc in
    let module S = Iter in
    ...
    if only_used
        then
            ...
            (* FROZENVAR *)
            pp_used_decls elo "FROZENVAR" out r;
            (* VAR *)
            pp_used_decls elo "VAR" out f;
            (* VAR / AUXILIARY *)
        else
            pp_all_decls elo out;
  ...
  let pp ~only_used ?(margin = 80) out { elo; init; invariant; trans; property } =
    ignore
      (pp_count_variables ~only_used ~margin out { elo; init; invariant; trans; property })

  (* write in temp file *)
  let make_model_file dir infile pp_generated model only_used =
    ...
    IO.with_out tgt (fun out ->
        ...
        nbvars := pp_count_variables ~only_used chan model);
    (tgt, !nbvars)
  ...
  let parse_enum (s : string) (i : int) : (Name.t * Tuple_set.t) =
      match Ltl.Atomic.lookup_enum s with
      | Some (name,pt) ->
          (
          match Enums.partial_vals pt with
          | Some (Some vs,ts) -> 
              let v = List.nth vs i in
              let fullv = Tuple_set.map (fun t -> Enums.tuple_to_full t pt) v in
              (name,fullv)
          | _ -> failwith (Printf.sprintf "parse_enum unexpected partial_tuple structure %s %i" s i)
          )
      | _ -> failwith (Printf.sprintf "parse_enum unrecognized %s %i" s i)
  ...
  let parseNuSmvOutput conversion_time analysis_time (cmd : string) elo_info (spec : string Gen.gen) = 
      ...
      let module P = Smv_trace_parser.Make (struct
        let base = musts
        let parse_enum = parse_enum
        let split_string = Ltl.Atomic.split_string
      end) in
      let trace =
        spec
        ...
        (*Printf.eprintf "%s\n" trace_str;*)
        try
            P.trace (Smv_trace_scanner.main) lexbuf
          with
          | P.Error ->
  ...
  let analyze ~conversion_time ~cmd ~script ~keep_files ~only_used ~no_analysis ~elo ~file
    ...
    let smv, nbvars = make_model_file dir file pp_generated model only_used in
    ...
    if no_analysis then (
      ...
      write_elo_to_file file nbvars elo; Printf.eprintf "done\n";
